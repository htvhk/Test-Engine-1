...
INFO_RE = re.compile(r"^info depth (?P<depth>\d+) seldepth (?P<seldepth>\d+) score cp (?P<score>-?\d+) nodes (?P<nodes>\d+) nps (?P<nps>\d+) hashfull (?P<hashfull>\d+) time (?P<time>\d+)(?: pv (?P<pv>.*))?$")
...
def run_engine(engine, position, adaptive):
    proc = subprocess.Popen([engine], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1)
    try:
        send(proc, "uci")
        ...
        if proc.returncode != 0 or info is None or stats is None or not bestmove or bestmove == "0000":
            raise RuntimeError(f"incomplete engine result rc={proc.returncode} info={info} stats={stats} bestmove={bestmove}")
        if int(info["depth"]) != DEPTH:
            raise AssertionError(f"depth {info['depth']} != {DEPTH}")
        return {
            "depth": int(info["depth"]), "seldepth": int(info["seldepth"]),
            "score_cp": int(info["score"]), "nodes": int(info["nodes"]),
            "qnodes": int(stats["qnodes"]), "tt_hits": int(stats["tthits"]),
            "beta_cutoffs": int(stats["cutoffs"]), "bestmove": bestmove,
            "pv": (info["pv"] or "").split(),
        }
    finally:
...
def main():
    ...
    args = p.parse_args()
    rows, drift, failures, ratios = [], [], [], []
    same_bestmove = same_score = 0
    for item in POSITIONS:
        ...
        rows.append({
            ...
            "baseline_main": baseline, "candidate_disabled": disabled,
            "candidate_enabled": enabled, "enabled_over_disabled_node_ratio": ratio,
        })
    ...
    out = {
        "schema": "TE1-ALPHA26-ADAPTIVE-LMR-R2-NODE-DIAGNOSTIC-v1",
        "depth": DEPTH, "positions": len(POSITIONS),
        ...
        "baseline_drift": drift, "candidate_failures": failures,
        "node_ratio_geometric_mean": geomean, "node_ratio_median": median,
        "same_bestmove_count": same_bestmove, "same_score_count": same_score,
        "decision": decision, "strength_claim_authorized": False, "rows": rows,
    }
    Path(args.output).write_text(json.dumps(out, indent=2) + "\n", encoding="utf-8", newline="\n")
    print(json.dumps({k: out[k] for k in ["decision", "node_ratio_geometric_mean", "node_ratio_median", "same_bestmove_count", "same_score_count", "baseline_drift", "candidate_failures"]}, indent=2))
